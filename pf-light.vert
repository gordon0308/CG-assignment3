...
in vec3 mtl_ambient;  // Ambient surface colour
in vec3 mtl_diffuse;  // Diffuse surface colour
in vec3 mtl_specular; // Specular surface colour
...
void main(void) {
    ...
    float fFogCoord = abs(a_vertex.z); 
   	...
   	vec4 eyePos = mv_matrix * vec4(a_vertex, 1.0);
	if(light_on){
    	colour = vec4(exponentialFog(phongPointLight(eyePos, eyeNorm), fFogCoord, 0.05), 1.0);
	}else{
		colour=vec4(exponentialFog((mtl_ambient),fFogCoord, 0.05),1.0);
